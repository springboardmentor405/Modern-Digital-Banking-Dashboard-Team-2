    ...
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      <nav className="bg-white border-b border-slate-200 px-6 py-4 flex justify-between items-center">
        ...
        <div className="flex items-center gap-4">
          <span className="text-slate-600 hidden sm:block">Welcome, <strong>{user?.name || "User"}</strong></span>
          <button 
      ...
      <main className="p-6 max-w-7xl mx-auto w-full space-y-6">
        <header>
          ...
          <p className="text-slate-500">Monitor your activities and balance</p>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            ...
            <div className="mt-2 text-xs text-green-600 font-medium">+2.5% from last month</div>
          </div>

          <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
            <p className="text-sm text-slate-500 mb-1">Monthly Spending</p>
            <h3 className="text-3xl font-bold text-slate-900">$3,240.50</h3>
            <div className="mt-2 text-xs text-slate-400">On track for budget</div>
          </div>
          ...
          <div className="bg-linear-to-r from-indigo-600 to-purple-700 p-6 rounded-2xl shadow-lg text-white">
            ...
            <div className="mt-4 w-full bg-white/20 rounded-full h-2">
              <div className="bg-white h-full rounded-full" style={{ width: '48%' }}></div>
            </div>
            <p className="mt-2 text-xs opacity-80">48% complete</p>
          </div>
        </div>

        {/* Transactions Placeholder List */}
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
          <h4 className="font-semibold mb-4 text-slate-800">Recent Transactions</h4>
            ...
            {[1, 2, 3].map((i) => (
              <div key={i} className="flex items-center justify-between py-3 border-b border-slate-50 last:border-0">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-medium text-slate-800">Payment to Vendor {i}</p>
